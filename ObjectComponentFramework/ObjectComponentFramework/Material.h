...
class Material : public rabd::Component<Material>
{
public:
	Material(){};
	~Material(){};
	...
	template<class T> ID3D11ShaderResourceView* GetTextureResource() {
		TextureType type = T::GetTextureTypeID();
		rabd::ObjectID id = textures[type];
	...
	inline ShaderLibrary::Shaders GetShader(){return shader;}
	inline void SetShader(ShaderLibrary::Shaders s){shader = s;}
private:
	std::map<TextureType, rabd::ObjectID> textures;		//  Store the texture IDs
